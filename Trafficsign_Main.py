...
'''Split Data'''
X_train, X_test, y_train, y_test = model_selection.train_test_split(images, classNo, test_size=testRatio)
...
assert (X_train.shape[0] == y_train.shape[0]), "No.of images in not equal to the number of labels in training set"
assert (X_validation.shape[0] == y_validation.shape[0]), "No.of images is not equal to number labels in validation set"
...
def my_model():
    t_model = Sequential()
    t_model.add(Conv2D(filters=32, kernel_size=(5, 5), activation='relu', input_shape=X_train.shape[1:]))
    ...
    # COMPILE MODEL
    t_model.compile(Adam(lr=0.001), loss='categorical_crossentropy', metrics=['accuracy'])
    return t_model
